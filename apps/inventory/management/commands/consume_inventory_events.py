...
class Command(BaseCommand):
    help = "Consume payment events and update inventory state."
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            ...
            extra={"topic": topic, "group_id": "inventory-service"},
        )
        ...
        try:
            while True:
                ...
                try:
                    ...
                    with transaction.atomic():
                        ...
                        if not created:
                            logger.info(
                                "Duplicate inventory event skipped",
                                extra={
                                    "event_id": event_id,
                        ...
                        if event_type == "payment.succeeded":
                            InventoryService.deduct_stock(items=payload["items"],order_id=payload["order_id"])

                        elif event_type == "payment.failed":
                            InventoryService.release_stock(items=payload["items"],order_id=payload["order_id"])
            ...
            logger.info("Inventory consumer stopped manually.")
        finally:
            consumer.close()